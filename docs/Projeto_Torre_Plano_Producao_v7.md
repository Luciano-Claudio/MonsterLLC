...
| 4 | 4 | Combat & Enemy Framework + Floor Sleep v1 | Combate com hordas reais + arquitetura de simulação seletiva | Vários inimigos diferentes causam dano ao jogador (contato/auto-disparo, cada um no seu cooldown, sem limite artificial de quantos simultâneos); Floors fora do atual não simulam integralmente | Enemy Framework (Melee/Ranged), timing/telegraph testado e substituído por contato/auto-disparo (Sprint 16 — Attack Budget removido), Population esqueleto, Projectile framework (+ modificadores pós-voo: gruda/explode/condição no chão/vira monstro/teleguiado), início real do Bestiary (2–3 monstros), **Floor Sleep/Activation v1** | Deadline 3 | 2+ tipos de monstro reais em cena com IA funcional causando dano por contato/projétil; população (não mais budget) é o único limitador de quantidade; **com 3 Floors existentes na Scene, somente o Current Floor executa simulação completa — Floors fora dele preservam Floor State (loot, Population State, Boss Timer, Floor Variant, Original Floor Identity, Active Floor Position) sem continuar rodando AI/Animator/pathfinding desnecessariamente** |
...
**Principais entregas:** Melee/Ranged genéricos, timing/telegraph de ataque + Attack Budget (Sprints 13-14) testados na prática contra 3 monstros reais na Sprint 16 — **contingência acionada**: modelo substituído por dano de contato/auto-disparo com cooldown próprio, IA de patrulha nova (`idle`/`walk` aleatório + `idle_combat`, GDD Seção 22), morte orientada por Animation Event; Attack Budget removido (GDD Seção 14); e a primeira versão de Floor Sleep/Activation: Current Floor com simulação completa, Floors fora dele com sistemas caros suspensos mas Floor State preservado (loot, Population State, Boss Timer, Floor Variant, Original Floor Identity, Active Floor Position).
...
**Por que agora?** Druid é o herói mais arriscado tecnicamente (transformação + conversão proporcional de HP) — melhor validar cedo que perto do fim. Boss Framework entra assim que a IA comum estiver madura (Deadline 4 concluída). **Nota pós-Sprint 16:** a maioria dos 30 bosses (definida ficha a ficha no Bestiário) reaproveita a base comum de contato/auto-disparo, só perdendo a animação `attack` dedicada — só um grupo pequeno e nomeado (Mother Slime Green/Blue, Rat People Royalty, Spider Queen, Dark Channeler, Lich, Dragon, Undead Dragon, Divine God) precisa da arquitetura própria por Animation Event. O Boss Framework desta Deadline deve nascer sabendo disso, não assumir Animation Event como padrão de todo boss.
...
| 16 | 4 | Teste de Combate Real + Pivot Arquitetural (correção) | Decisão de arquitetura de combate comum tomada e implementada | Rat/Goblin/Rat People (os 3 do teste) rodando no modelo definitivo: Melee por contato e Ranged por auto-disparo, cada um só com cooldown próprio; IA de patrulha nova (`idle`/`walk` aleatório antes de detectar, nunca interrompendo `idle` no meio do clipe) + `idle_combat`; morte só destrói/dropa loot ao fim do clipe `die` (Animation Event + timeout de segurança); animações `idle/walk/idle_combat/damage/die`; `AttackBudgetTracker`/`AttackBudgetManager`/`AttackType` removidos do projeto; GDD Seção 22/14 atualizadas para refletir a decisão. **Sem produção de Bestiário nova** — ver nota abaixo. | Sprint 15 |
...
| 20 | 5 | Mage — Ultimate (completa Mage) + Floor 1A–2A + Bestiary Batch 1+2 completo (Floors 1–2, todo o elenco) | Mage completo + primeiros Floors reais habitados e povoados | Fireball explode e deixa rastro persistente; Floor 1A e 2A jogáveis com layout e inimigos reais; Floors 1–2 com o elenco completo (Wolf, Bat, Slime Green/Blue, Goblin Raider, Goblin Sapper, Centaur, Minotaur, Gnoll, Spider, Ancient Troll — além de Rat/Goblin/Rat People já prontos desde a Sprint 16) | Sprint 19 |
...
**Bestiary:** Batch 1+2 combinado (S20 — Floors 1–2 completo; Rat/Goblin/Rat People já prontos desde a correção da S16, inclui a exceção Goblin Sapper) · Batch 3 (S22 — Floors 3–4, inclui a exceção Orc Shaman) · Batch 4 (S25 — Floors 5–6, maior volume da tabela: Floor 5 sozinho tem 20 fichas; inclui a exceção Burning Skull; Skeleton Rider deixou de ser exceção) · Batch 5 (S31 — Floors 7–10, inclui as exceções Serpent e Bicephalous) · Leftovers (S49). ✅ Distribuído — **única mudança real de distribuição desta revisão:** o Batch 1 original (S16, Deadline 4) foi absorvido pelo Batch 2 (S20, Deadline 5), porque a correção de framework sozinha já ocupa a Sprint 16 inteira (ver nota na Deadline 4). Isso não fere o Exit Criteria da Deadline 4 (que não exige conteúdo novo de Bestiário, só IA funcional em 2+ monstros). A partir da Sprint 16 o custo por ficha caiu para a maioria das criaturas (sem Telegraph/Attack Budget), exceto nas 5 exceções comuns documentadas no Bestiário e nas 8 exceções de boss (arquitetura própria por Animation Event, custo equivalente ao modelo antigo) — folga esperada em cada Batch deve ser tratada como margem, não como convite a adiantar conteúdo por conta própria (ver nota de versão no topo do documento).
