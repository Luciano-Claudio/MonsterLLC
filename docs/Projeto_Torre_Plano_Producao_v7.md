...
**Estado (pós-reversão do combate híbrido):** ainda não confirmada 100% fechada. Duas pendências abertas: (1) Spectre (boss Andar 5) sinalizado 🟡 no Bestiário — só 3 animações no total, dar `attack` de volta quebraria o design minimalista; decisão explícita ainda não tomada, não tratar como resolvido; (2) Floor Sleep (Sprint 15) precisa de um re-teste manual depois desta última rodada de mudanças no Enemy Framework — não deveria ter quebrado nada, mas ainda não foi testado explicitamente sobre a arquitetura híbrida final.
...
**Por que agora?** Druid é o herói mais arriscado tecnicamente (transformação + conversão proporcional de HP) — melhor validar cedo que perto do fim. Boss Framework entra assim que a IA comum estiver madura (Deadline 4 concluída). **Nota pós-Sprint 16 (correção):** a maioria dos 30 bosses (definida ficha a ficha no Bestiário) reaproveita a base comum híbrida — **mantém a animação `attack` real com Animation Event**, só perdendo a complexidade extra que tinha antes (telegraph, dano em área, múltiplos hits, sequências). Só um grupo pequeno e nomeado (Mother Slime Green/Blue — só contato, igual Slime comum; Rat People Royalty, Spider Queen, Dark Channeler, Lich, Dragon, Undead Dragon, Divine God) precisa de arquitetura própria além do padrão. O Boss Framework desta Deadline deve nascer sabendo que **todo boss (exceto Mother Slime) usa Animation Event pro ataque** — a diferença entre boss comum e boss-exceção não é "tem attack ou não", é "a mecânica do attack é genérica ou bespoke".
...
**Bestiary:** Batch 1+2 combinado (S20 — Floors 1–2 completo; Rat/Goblin/Rat People já prontos desde a correção da S16, inclui a exceção Goblin Sapper) · Batch 3 (S22 — Floors 3–4, inclui a exceção Orc Shaman) · Batch 4 (S25 — Floors 5–6, maior volume da tabela: Floor 5 sozinho tem 20 fichas; inclui a exceção Burning Skull; Skeleton Rider deixou de ser exceção) · Batch 5 (S31 — Floors 7–10, inclui as exceções Serpent e Bicephalous) · Leftovers (S49). ✅ Distribuído — **única mudança real de distribuição desta revisão:** o Batch 1 original (S16, Deadline 4) foi absorvido pelo Batch 2 (S20, Deadline 5), porque a correção de framework sozinha já ocupa a Sprint 16 inteira (ver nota na Deadline 4). Isso não fere o Exit Criteria da Deadline 4 (que não exige conteúdo novo de Bestiário, só IA funcional em 2+ monstros). **Premissa de custo corrigida (decisão final do combate, pós-reversão híbrida):** todo monstro comum e a maioria dos bosses voltou a precisar de uma animação `attack`/conjuração real por Animation Event — não é mais "sem Animator" como a nota anterior desta linha dizia. O corte de custo real vem da ferramenta `MonsterAnimationGeneratorWindow` (Sprint 16): clipes direcionais gerados a partir de sprite sheet sobre 2 Animators base compartilhados (`Base_Melee`/`Base_Ranged`, `AnimatorOverrideController`), com `AnimationHitEvent`/`AnimationAttackEndEvent`/`AnimationDieEndEvent` inseridos automaticamente na criação — configurar estado/transição do zero por criatura, que era o custo caro original, não existe mais. As exceções continuam exigindo trabalho bespoke além do que a ferramenta gera sozinha: 5 comuns (Goblin Sapper, Orc Shaman — só o totem, Burning Skull, Serpent, Bicephalous) e 8 bosses (Mother Slime Green/Blue — cai na exceção permanente dos Slimes, contato sem `attack`; Rat People Royalty, Spider Queen, Dark Channeler, Lich, Dragon, Undead Dragon, Divine God — mecânica própria além do `attack` padrão). Spectre (boss Andar 5) é uma pendência 🟡 à parte, ainda não decidida (ver Riscos/Estado da Deadline 4) — não contar como exceção fechada nem como padrão até a decisão sair. Folga esperada em cada Batch (pela ferramenta, não pela ausência de `attack`) deve ser tratada como margem, não como convite a adiantar conteúdo por conta própria (ver nota de versão no topo do documento).
