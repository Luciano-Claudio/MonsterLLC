...
- Herdada (Sprint 11): nada ainda pausa/congela o mundo quando o estado sai de `GameState.Gameplay` — sem sprint formalmente atribuída; prioridade entre interagíveis simultâneos (Sprint 35).
...
Com Save real, Continue Game correto e o ciclo de dia validado de ponta a ponta (sucesso e falha), a **Deadline 3 (Primeiro Vertical Slice, Sprints 9–12) está encerrada**: o jogo agora sustenta uma run completa — Menu, criação de run, combate, loot, venda, demanda, resultado, loja mínima e checkpoint real — sem nenhum passo manual escondido no meio. A Deadline 4 (Enemy Framework + Floor Sleep v1, Sprints 13–16) começa a expandir o combate para uma base genérica de inimigos (Melee/Ranged configuráveis) e a simulação seletiva por Floor, deixando o vertical slice atual como fundação em vez de protótipo descartável.
