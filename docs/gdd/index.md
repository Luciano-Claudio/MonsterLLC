...
Com hordas grandes, todo Melee tentando ficar dentro do `attackRadius` ao mesmo tempo lota o corpo a corpo e fica ilegível. Existe um teto global e configurável (`MeleeAttackSlotManager`, padrão **12**, ajustável no Inspector) de quantos Melee podem estar em alcance de contato do jogador ao mesmo tempo. Um Melee que chega no `flankRadius` (um raio maior que o `attackRadius`, também configurável por monstro) sem conseguir uma vaga fica **flanqueando**: alterna entre andar na borda desse anel ao redor do jogador e parar em `idle_combat`, tentando de novo a cada frame até uma vaga liberar (quando outro Melee com vaga morre). Ranged não participa disso — já mantém distância própria, não lota o corpo a corpo.
